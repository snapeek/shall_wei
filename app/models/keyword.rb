...
class Keyword
  ...
  def baidu_news_to_csv
    CSV.open("tmp/csv/#{content}-百度新闻-#{Time.at(starttime)}-#{Time.at(endtime)}.csv", "wb") do |csv|
      ...
      baidu_news.each do |bn|
        csv << [bn.title, bn.summary, bn.url, bn.source, Time.at(bn.created_at), (bn.lcountents.count rescue 0) , (bn.lcountents.map { |e| e["source"] }.join(' ') rescue '')]
      end
    end
  end

  def day_count_to_csv
    CSV.open("tmp/csv/#{content}-微博每日提及数-#{Time.at(starttime)}-#{Time.at(endtime)}.csv", "wb") do |csv|
      csv << ["日期", "提及数"]
  ...
  def weibo_to_csv
    CSV.open("tmp/csv/#{content}-微博列表-#{Time.at(starttime)}-#{Time.at(endtime)}.csv", "wb") do |csv|
  ...
  def repost_to_scv
    CSV.open("tmp/csv/#{content}-微博转发路径-#{Time.at(starttime)}-#{Time.at(endtime)}.csv", "wb") do |csv|
